...
def get_model(arch='resnet18', eps=0, checkpoint_dir="./pretrained-models", inet_dir="./data"):
...
def get_inet_label_index_pair(inet_dir="./data", split="val"):
    dset = ImageFolder(os.path.join(inet_dir, split))
    label_index_pair = {i: [] for i in range(100)}
...
def get_batch_index(anchor_label, anchor_index, label_index_pair, batch_size, num_batches):
    ...
    label_list = list(set(range(100)) - set([anchor_label]))
...
if __name__ == "__main__":
    net = get_model('resnet50', eps=3)
    print(isinstance(net.model, robustness.imagenet_models.vgg.VGG))
